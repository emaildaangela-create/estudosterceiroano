...
(function () {
  ...
  function abrirRevisaoExtra(nome){if(typeof REVISOES_EXTRAS==='undefined'||!REVISOES_EXTRAS[nome])return;var revisao=REVISOES_EXTRAS[nome],raiz=$('#revisao-conteudo');$('#revisao-titulo').textContent=revisao.titulo;raiz.innerHTML=revisao.html+'<section class="gabarito-responsavel"><p class="gabarito-responsavel__area">Área do responsável</p><h2>Gabarito para correção</h2><p>Respostas conferidas a partir dos enunciados, sem copiar as respostas manuscritas.</p><button class="gabarito-responsavel__botao" type="button" aria-expanded="false">Mostrar gabarito</button><div class="gabarito-responsavel__conteudo" hidden>'+revisao.gabarito+'</div></section><button class="revisao-limpar" type="button">Limpar respostas</button>';$$('.revisao-abas [data-revisao]').forEach(function(b){b.setAttribute('aria-selected',b.dataset.revisao===nome?'true':'false');});var botaoGabarito=raiz.querySelector('.gabarito-responsavel__botao'),conteudoGabarito=raiz.querySelector('.gabarito-responsavel__conteudo');botaoGabarito.addEventListener('click',function(){var abrir=conteudoGabarito.hidden;conteudoGabarito.hidden=!abrir;botaoGabarito.setAttribute('aria-expanded',abrir?'true':'false');botaoGabarito.textContent=abrir?'Ocultar gabarito':'Mostrar gabarito';});raiz.querySelector('.revisao-limpar').addEventListener('click',function(){raiz.querySelectorAll('input').forEach(function(c){if(c.type==='radio'||c.type==='checkbox')c.checked=false;else c.value='';});raiz.querySelectorAll('textarea').forEach(function(c){c.value='';});toast('Respostas apagadas.');});mostrar('tela-revisoes');}
