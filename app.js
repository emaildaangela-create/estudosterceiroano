...
(function () {
  ...
  function renderPilotoVisual(stage,cap,game,passo,perfil,concluir) {
    ...
    else if(['origins','clockwork','observatory','comics','mailroom','sharing','moonlog'].indexOf(perfil.pilot)>=0)renderPilotoPares(stage,cena,game,passo,concluir,perfil.pilot);
    else if(['ruralurban','access','wordrepair','pollutionlab','citycouncil'].indexOf(perfil.pilot)>=0)renderPilotoClassificar(stage,cena,game,passo,concluir,perfil.pilot);
  }
  ...
  function renderPilotoPares(stage,cena,game,passo,concluir,tipo) {
    var par=game.pairs[passo], respostas=game.pairs.map(function(p){return p[1];}), icones={origins:'🧳',clockwork:'🕰️',observatory:'🔭',comics:'💬',mailroom:'✉️',sharing:'🧺',moonlog:'🌙'}, icone=icones[tipo]||'✨';
    var chamadas={origins:'Lugar de origem',clockwork:'Horário no cotidiano',observatory:'Corpo celeste',comics:'Cena dos quadrinhos',mailroom:'Forma de tratamento',sharing:'Desafio de divisão',moonlog:'Registro da Lua'};
  ...
  function renderPilotoClassificar(stage,cena,game,passo,concluir,tipo) {
    var item=game.items[passo], nomes=game.categories, tres=nomes.length===3, muitos=nomes.length>3, icones={access:'🔑',ruralurban:'🧭',wordrepair:'✏️',pollutionlab:'🚨',citycouncil:'🏙️'};
